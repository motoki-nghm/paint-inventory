...
- Yahoo Shopping API key is server-side only (`YAHOO_APP_ID` env var).
...
| API          | `/api/yahoo-lookup` validates JAN, rate-limits per IP, hides upstream   |
...
- The in-memory rate limit on the Yahoo proxy is best-effort. Multiple instances
